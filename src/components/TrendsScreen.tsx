    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FFF4FF] via-[#FFEFE5] to-[#FFF6F0]">
      <div className="max-w-lg mx-auto px-6 py-6">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {trendingAudio.map((audio) => (
              <div key={audio.id} className="bg-white rounded-2xl p-4 shadow-md hover:shadow-lg transition-all cursor-pointer">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-[#7D7D7D] mt-2">
                      ...
                      <span className="text-[#2D9CDB] flex items-center gap-1">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {trendingStyles.map((style) => (
              <div key={style.id} className="bg-white rounded-2xl p-4 shadow-md hover:shadow-lg transition-all cursor-pointer">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="text-[#3A3A3A] flex-1">{style.style}</h3>
                  <div className="flex items-center gap-1 text-sm text-[#2D9CDB] flex-shrink-0 ml-3">
                    <TrendingUp className="w-4 h-4" strokeWidth={2} />
                    {style.performance}%
                  </div>
                </div>
                <p className="text-sm text-[#7D7D7D] mb-3">{style.description}</p>
                <div className={`bg-gradient-to-r ${style.gradient} bg-opacity-10 px-3 py-2 rounded-xl inline-block`}>
                  <span className="text-xs text-[#3A3A3A]">Best for: {style.bestFor}</span>
                </div>
              </div>
            ))}
          </div>
              ...
              {bestPostingTimes.map((slot, index) => (
                <div key={index} className={`flex items-center justify-between py-3 ${
                  ...
                  <div className="flex-1">
                    <p className="text-[#3A3A3A] text-sm mb-1">{slot.time}</p>
                    <p className="text-xs text-[#7D7D7D]">{slot.reason}</p>
                  </div>
                  <span className={`${slot.color} text-white text-xs px-3 py-1 rounded-full flex-shrink-0 ml-3`}>
                    {slot.engagement}
                  </span>
                </div>
              ))}
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {localTrends.map((trend) => (
              <div key={trend.location} className="bg-white rounded-2xl p-4 shadow-md hover:shadow-lg transition-all cursor-pointer">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <p className="text-sm text-[#A55BFF] mb-1 flex items-center gap-1">
                  ...
                  <div className="text-right ml-3">
                    ...
                    <p className="text-sm text-[#3A3A3A]">{trend.posts}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-gradient-to-r from-[#A55BFF] to-[#FF4FD1] text-white rounded-2xl p-5 shadow-xl">
          ...
          <button className="bg-white text-[#A55BFF] px-5 py-3 rounded-xl text-sm hover:bg-opacity-90 transition-all flex items-center gap-2">
            Use This Template
